...
# Helper function to mark IP as released
mark_ip_released() {
  local ip=$1
  if [ -z "$RELEASED_IPS" ]; then
    RELEASED_IPS="$ip"
  else
    RELEASED_IPS="$RELEASED_IPS $ip"
  fi
}
...
if [ "${is_dynamic_vms}" == "true" ]; then
  ...
  QE_SERVER_MANAGER_URL="http://172.23.217.21:8081"
