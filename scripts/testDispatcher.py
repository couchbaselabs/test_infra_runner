...
def extract_individual_tests_from_query_result(col_rel_version,
    ...
    def sub_component_is_supported_in_this_release(data_from_db_doc):
        ...
        if 'maxVersion' in data_from_db_doc:
            if columnar_rel_version:
                if columnar_rel_version > float(
                        data_from_db_doc['maxVersion']):
                    print(str(data_from_db_doc['subcomponent']),
                          ' is not supported in this release')
                    return False
            elif RELEASE_VERSION > float(data_from_db_doc['maxVersion']) \
                    or (col_rel_version
    ...
    def populate_required_dispatcher_data(data_from_db_doc,
        ...
        if data["component"] == "columnar" \
                and str(columnar_rel_version).lower() not in ["none", ""]:
    ...
    # CBQE-8380
    global options, RELEASE_VERSION, REPO_PULLED, columnar_rel_version
    ...
    if options.branch != "master":
        try:
            ...
            print('Git error: Did not find {} in {} branch'.format(
    ...
    ini_uname, ini_password = get_ssh_username_password(data['config'])
    common_sub_comp_dict = {
        ...
        'ssh_username': ini_uname,
        'ssh_password': ini_password,
    ...
    if options.component is None or options.component == 'None':
        ...
        if (options.subcomponent is None or options.subcomponent == 'None') \
            ...
            queryString = (
                ...
                .format(suiteString, componentString,
                        options.subcomponent_regex)
                ...
                if options.serverType in [
                    ...
                    if "cloud.couchbase.com" in options.capella_url:
                        ...
                        invited_user, invited_password = capella.invite_user(
                            options.capella_token,options.capella_url, options.capella_user, options.capella_password, options.capella_tenant)
