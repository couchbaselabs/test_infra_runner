...
if [ 0 == 1 ]; then
  ...
  wget -O dispatcher.out  "http://qa.sc.couchbase.com/job/test_suite_dispatcher/buildWithParameters?token=extended_sanity&OS=windows22&retries=1&version_number=$version_number&suite=12hr_upgrade&component=fts&subcomponent=None&url=$url&serverPoolId=$serverPoolId&addPoolId=elastic-fts&branch=$branch&use_dynamic_vms=true&extraParameters=$extraParameters,bucket_storage=magma"
